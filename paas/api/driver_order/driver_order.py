...
def fetch_current_order():
    ...
    if user == "Guest": frappe.throw("Unauthorized")
    
    order = frappe.get_list('Order', 
                            filters={'deliveryman': user, 'status': ['in', ['On a Way', 'Accepted']]}, 
...
def upload_order_image(order_id, image_url=None):
    if not image_url: return {"status": False}
    if frappe.db.exists('Order', order_id):
        doc = frappe.get_doc('Order', order_id)
