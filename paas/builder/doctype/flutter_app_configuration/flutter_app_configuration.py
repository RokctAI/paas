	...
	def onload(self):
		...
		try:
			if not self.google_api_key:
				self.google_api_key = frappe.db.get_single_value("Location Settings", "google_map_key")
		except Exception:
			pass
		...
		try:
			if not self.firebase_web_key:
				self.firebase_web_key = frappe.db.get_single_value("Push Notification Settings", "api_key")
		except Exception:
			pass
		...
		try:
			...
			if not (self.payfast_merchant_id and self.payfast_merchant_key and self.payfast_passphrase):
