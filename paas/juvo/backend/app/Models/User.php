...
class User extends Authenticatable implements MustVerifyEmail
{
    ...
    */
    ...
    /**
     * Get all user memberships (active and inactive).
     */
    ...
    // Add the booted method here
    protected static function booted()
    {
        static::addGlobalScope('membership', function ($query) {
            $query->with([
                'userMembership' => function($q) {
    ...
    public function scopeFilter($query, array $filter) {
        ...
        $query
            ...
            ->when(data_get($filter, 'shop_id') && request('role') === 'deliveryman', function ($query, $shopId) {
                $shop = Shop::find($shopId);
                if ($shop) {
                    $requiredVehicles = $shop->getRequiredVehicleTypes();
                    $query->whereHas('deliveryManSetting', function($q) use ($requiredVehicles) {
                        $q->whereIn('type_of_technique', $requiredVehicles);
                    });
                }
            })
