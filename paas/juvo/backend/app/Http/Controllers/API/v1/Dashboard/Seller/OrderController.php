...
class OrderController extends SellerBaseController
{
    ...
    /**
     * Update Order DeliveryMan Update with Smart Vehicle Filtering.
     *
     * @param int $orderId
    ...
    {
        $order = Order::with('shop')->where('shop_id', $this->shop->id)->findOrFail($orderId);

        // Validate that the delivery man has the right vehicle for this shop type
        $deliverymanId = $request->input('deliveryman');
        $user = User::with('deliveryManSetting')->find($deliverymanId);

        if (!$user || !$user->deliveryManSetting) {
            return $this->onErrorResponse([
        ...
        $requiredVehicles = $order->shop->getRequiredVehicleTypes();
        $userVehicle = $user->deliveryManSetting->type_of_technique;

        if (!in_array($userVehicle, $requiredVehicles)) {
            $shopType = $order->shop->type_display;
            return $this->onErrorResponse([
                'code' => ResponseError::ERROR_400,
                'message' => "Selected delivery person's vehicle ({$userVehicle}) is not suitable for {$shopType}. Required vehicles: " . implode(', ', $requiredVehicles)
            ]);
        }

        $result = $this->orderService->updateDeliveryMan($orderId, $deliverymanId, $this->shop->id);
