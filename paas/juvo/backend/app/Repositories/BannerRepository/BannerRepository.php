...
class BannerRepository extends CoreRepository
{
	...
	public function bannersPaginate(array $filter): mixed
    {
        ...
        $currentLocale = $this->language ?? $locale;
        ...
        return $this->model()
            ...
            ->with([
                'translation' => fn($q) => $q
                    ...
                    ->where(fn($q) => $q->where(fn($q) => 
                        $q->where('locale', $currentLocale)
                        ->orWhere('locale', $locale)
                    ))
                    ->select('id', 'banner_id', 'locale', 'title', 'button_text')  // Add button_text to select
            ])
            ->when(data_get($filter, 'search'), function ($query, $search) use ($locale, $currentLocale) {
                $query->whereHas('translations', function ($q) use ($search, $locale, $currentLocale) {
                    $q
	...
	public function adsPaginate(array $filter): array
	{
		...
		$adsPackages = ShopAdsPackage::with([
			'banner.translation' => fn($q) => $q
				...
				->select('id', 'banner_id', 'locale', 'title', 'button_text')
		])
