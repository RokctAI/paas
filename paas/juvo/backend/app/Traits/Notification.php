...
trait Notification
{
    private function handleFailedNotification(string $token, int $status): void
{
    if ($status === 403 || $status === 404) {
        try {
            // Simple update for non-JSON tokens
            User::where('firebase_token', $token)->update(['firebase_token' => null]);
            
            // Update for JSON array tokens by using LIKE
            User::where('firebase_token', 'LIKE', '%' . $token . '%')
            ...
            Log::error('Failed to clean up token: ' . $e->getMessage(), [
                'token' => $token,
                'status' => $status
            ]);
        }
        ...
        foreach ($receivers as $receiver) {
            try {
                dispatch(function () use ($receiver, $message, $title, $data, $firebaseTitle, $headers, $url) {
                    ...
                    $request = Http::withHeaders($headers)->post($url, [
                        'message' => [
                            'token' => $receiver,
                            'notification' => [
                    ...
                    // Add token cleanup for failed notifications
                    if ($request->status() === 403 || $request->status() === 404) {
                        $this->handleFailedNotification($receiver, $request->status());
                    }
