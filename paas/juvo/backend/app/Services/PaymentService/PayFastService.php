	...
	public function orderProcessTransaction(array $data): PaymentProcess|array
{
    ...
    // Create a single unique UUID for this payment process and use it consistently
    ...
    // Use the UUID in the notify URL
    ...
    $body = [
        ...
        // IMPORTANT: Use the UUID as m_payment_id to ensure consistent tracking
        'm_payment_id'  => $uuid,
        ...
        // Store model type and ID in custom field for easier lookup in webhook
        'custom_str1' => strtolower(str_replace('App\\Models\\', '', data_get($before, 'model_type'))) . ':' . $modelId,
        'custom_str2' => 'tokenize',
        'custom_str4' => data_get($data, 'card_last_four') ?? '••••',
    ];
    
    // Add special return parameters to capture the token
    $body['return_url'] = $url . '&capture_token=true';
    ...
    if (data_get($data, 'type') === 'mobile') {
        ...
        return PaymentProcess::updateOrCreate([
            ...
            'data' => array_merge([
                ...
                'm_payment_id' => $uuid, // Store for reference
                'uuid' => $uuid,  // Add this to help with webhook matching
            ], $before, $body),
        ]);
    }

    // Generate signature and add it to the body
    ...
    // Create and return payment process WITH THE UUID AS ID
    return PaymentProcess::updateOrCreate([
        'user_id'    => auth('sanctum')->id(),
        'model_id'  => $modelId,
        'model_type' => data_get($before, 'model_type')
    ], [
        'id' => $uuid, // Use our UUID consistently as the ID
        'data' => array_merge([
            'price'      => $totalPrice,
            'payment_id' => $payment?->id,
            'sandbox'    => $payload['sandbox'],
            'signature'  => $body["signature"],
            'uuid'       => $uuid, // Store UUID in data too for reference
            'm_payment_id' => $uuid, // Store the same ID we sent to PayFast
        ], $before, $response),
    ]);
}
	...
	public function splitTransaction(array $data): PaymentProcess|array
	{
		...
		for ($i = 0; $split > $i; $i++) {
			...
			$body = [
				...
				// Add these for tokenization (only for first payment in split)
    			'subscription_type' => 2, 
    			'payment_method' => 'cc',
			];
	...
	public function subscriptionProcessTransaction(array $data, Shop $shop): PaymentProcess
	{
		...
		$body = [
			...
			'email_address' => $user->email ?? Str::random() . '@gmail.com',
			 ...
			 // Force credit card for subscription (already has subscription_type)
    		'payment_method' => 'cc',
		]; 
